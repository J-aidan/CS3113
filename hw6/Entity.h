...
enum EntityType { PLATFORM, PLAYER, ENEMY, PPROJECTILE, EPROJECTILE, BACKGROUND };
enum AIType     { WALKER, GUARD, PATROLLER, JUMPER, FLYER, SHOOTER, VSHOOTER, ASTEROID };
...
private:
    ...
    int m_walking[2][14]; // array for walking animations
...
public:
    ...
    // ————— METHODS ————— //
    Entity();
    Entity(GLuint texture_id, float speed, glm::vec3 acceleration, float jump_power, int walking[2][14], float animation_time,
    ...
    void ai_shoot();
    void ai_vshoot();
    void ai_asteroidfly(Entity* player);
    ...
    void const shoot() { m_shooting = true; }
    ...
    bool      const get_is_shooting()     const { return m_shooting; }

    int       const get_enemies_killed() const { return m_enemies_killed; }
    ...
    void const set_init_position(glm::vec3 new_position) { m_init_position = new_position; }
    ...
    void const set_hp(int val) { m_hp = val; }
    void const set_shooting(bool val) { m_shooting = val; }

    // Setter for m_walking
    void set_walking(int walking[2][14])
    {
        for (int i = 0; i < 2; ++i)
        {
            for (int j = 0; j < 14; ++j)
